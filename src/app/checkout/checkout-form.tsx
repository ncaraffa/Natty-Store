...
type CheckoutResponse = {
  message?: string;
  pix?: PixInfo;
  order?: { id: string; reservationExpiresAt: string | null };
};

type OrderPixResponse = {
  message?: string;
  pix?: PixInfo;
...
export default function CheckoutForm() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/orders/${targetOrderId}/pix`)
      .then((res) => res.json().catch(() => null) as Promise<OrderPixResponse | null>)
      .then((data) => {
        if (data?.pix) {
          setPix(data.pix);
          setPendingOrder(targetOrderId, data.pix.expiresAt);
        } else {
          // Pedido já foi pago, rejeitado, expirou, ou não existe mais:
          // não há mais nada para retomar aqui.
          clearPendingOrder();
          if (data?.paymentStatus && data.paymentStatus !== "pending") {
            setMessage("Este pedido já foi processado. Confira o status em Minhas compras.");
          }
        }
      })
      .catch(() => {
  ...
  useEffect(() => {
    ...
    if (estimatedTotal > 0) setPixTotalCents(estimatedTotal);
  }, [pix, pixTotalCents, lines, products]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.ok && data?.pix) {
        ...
        // Mantém os itens no carrinho até o Pix ser pago ou expirar, para
        // que o cliente consiga voltar exatamente a esta tela depois.
        if (data.order?.id) {
          setPendingOrder(
            data.order.id,
            data.pix.expiresAt ?? data.order.reservationExpiresAt ?? null,
          );
        }
    ...
    return (
      <section className="narrow">
        ...
        <div className="notice" role="status" aria-live="polite">
          Carregando seu pagamento pendente…
        </div>
      </section>
    );
  }
        ...
        <div className="pix-panel">
          <div className="panel">
            ...
            <div className="warning">
              Fechou esta página sem querer? Sem problema: seus itens ficam
              reservados e você pode voltar aqui a qualquer momento por{" "}
              <Link href="/minhas-compras">Minhas compras</Link>. A confirmação
              é automática pelo Mercado Pago assim que o Pix for aprovado.
            </div>
