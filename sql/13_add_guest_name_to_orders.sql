-- Adiciona o nome do cliente ao checkout de convidado. Até aqui só coletávamos nick do Roblox
-- e contato em orders (guest_roblox_nick/guest_contact); custom_requests já tinha "name" desde
-- o início. Esta migração alinha o checkout ao mesmo padrão.
--
-- ATENÇÃO: esta migração ALTERA PRODUÇÃO (tabela orders com clientes comprando agora) e a função
-- reserve_checkout usada por todo checkout ativo. Ela recria reserve_checkout a partir da versão
-- restaurada em 10_restore_reserve_checkout.sql (a versão real hoje em produção, confirmada via
-- pg_get_functiondef), apenas acrescentando p_guest_name como parâmetro novo no final, com default
-- null — mesmo padrão usado em 10_restore_reserve_checkout.sql ao acrescentar p_coupon_code.
-- Aplicar manualmente via SQL editor do Supabase somente após revisão.
begin;

alter table public.orders add column if not exists guest_name text;

drop function if exists public.reserve_checkout(text,text,text,jsonb,uuid,interval,text);
...
begin
  ...
  if p_guest_name is not null
     and char_length(btrim(p_guest_name)) not between 1 and 120 then
...
revoke all on function public.reserve_checkout(text,text,text,jsonb,uuid,interval,text,text) from public, anon, authenticated;
grant execute on function public.reserve_checkout(text,text,text,jsonb,uuid,interval,text,text) to service_role;
