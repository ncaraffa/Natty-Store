-- Corrige reserve_checkout: a coluna orders.checkout_fingerprint (not null, texto) existia em produção
-- fora do schema.sql versionado e foi perdida ao recriar a função para os cupons (08_coupons.sql).
-- Reconstruída a partir do formato observado em pedidos reais: {"items":[...],"contact":...,"roblox_nick":...,"auth_user_id":null}
begin;
...
declare
 ...
 v_subtotal_cents integer; v_discount_cents integer := 0; v_coupon public.coupons; v_coupon_code text; v_fingerprint text;
begin
 ...
 select jsonb_build_object(
   'items', (select jsonb_agg(jsonb_build_object('product_id', product_id, 'quantity', quantity) order by product_id) from checkout_lines),
 ...
 insert into public.orders(id,guest_roblox_nick,guest_contact,status,total_cents,coupon_code,discount_cents,idempotency_key,checkout_fingerprint,reservation_expires_at)
 values (v_order_id,btrim(p_guest_roblox_nick),btrim(p_guest_contact),'stock_reserved',v_subtotal_cents-v_discount_cents,v_coupon_code,v_discount_cents,p_idempotency_key,v_fingerprint,v_expires_at);
